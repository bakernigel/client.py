...
class EcovacsLastJobEventEntity(
    ...
    async def async_added_to_hass(self) -> None:
        ...
        async def on_event(event: ReportStatsEvent) -> None:
            """Handle event."""
            if event.status in (
                CleanJobStatus.NO_STATUS,
                CleanJobStatus.CLEANING,
            ):
                # Trigger only when the job has ended.
                return
            ...
            self._trigger_event(
                event_type,
                {
                    "area": event.area,
                    "duration": event.time,
                    "cleaning_type": event.type,
                    "cleaning_id": event.cleaning_id,
                    "rooms": event.content,
                },
            )
