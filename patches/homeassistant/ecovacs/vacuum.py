...
_ATTR_ROOMS = "rooms"
_ATTR_SELECTED_ROOMS = "selected_rooms"
_ATTR_ROOM_PROGRESS = "room_progress"
_ATTR_CURRENT_ROOM = "current_room"
_ATTR_COMPLETED_ROOMS = "completed_rooms"
...
class EcovacsVacuum(
    ...
    _unrecorded_attributes = frozenset(
        {
            _ATTR_ROOMS,
            _ATTR_SELECTED_ROOMS,
            _ATTR_ROOM_PROGRESS,
            _ATTR_CURRENT_ROOM,
            _ATTR_COMPLETED_ROOMS,
        }
    )
    ...
    def __init__(self, device: Device) -> None:
        ...
        self._selected_rooms_event: SelectedRoomsEvent | None = None
        self._cleaning_progress_event: CleaningProgressEvent | None = None
    ...
    async def async_added_to_hass(self) -> None:
        ...
        if selected_rooms_cap := self._capability.clean.selected_rooms:

            async def on_selected_rooms(event: SelectedRoomsEvent) -> None:
                self._selected_rooms_event = event
                self.async_write_ha_state()

            self._subscribe(selected_rooms_cap.event, on_selected_rooms)

        if progress_cap := self._capability.clean.progress:

            async def on_cleaning_progress(event: CleaningProgressEvent) -> None:
                self._cleaning_progress_event = event
                self.async_write_ha_state()

            self._subscribe(progress_cap.event, on_cleaning_progress)
    ...
    def extra_state_attributes(self) -> Mapping[str, Any] | None:
        ...
        if self._capability.clean.selected_rooms is not None:
            attributes[_ATTR_SELECTED_ROOMS] = (
                list(self._selected_rooms_event.rooms)
                if self._selected_rooms_event is not None
                else []
            )

        if self._capability.clean.progress is not None:
            room_progress: dict[int, str] = {}
            current_room: int | None = None
            completed_rooms: list[int] = []

            if self._cleaning_progress_event is not None:
                for room in self._cleaning_progress_event.rooms:
                    status = room.status.name.lower()
                    room_progress[room.room_id] = status

                    if status == "cleaning":
                        current_room = room.room_id
                    elif status == "finished":
                        completed_rooms.append(room.room_id)

            attributes[_ATTR_ROOM_PROGRESS] = room_progress
            attributes[_ATTR_CURRENT_ROOM] = current_room
            attributes[_ATTR_COMPLETED_ROOMS] = completed_rooms
